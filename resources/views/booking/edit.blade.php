...
<body>
    ...
    <div class="card shadow-sm border-0 rounded-lg">
        ...
        <div class="card-header text-center">
            ...
            <h4 class="card-title mt-3">Edit Your Appointment</h4>
        </div>

        <div class="card-body">
            <form id="bookingForm" method="POST" action="{{ route('booking.update', $booking->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="step active" id="step1">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input id="contact" type="text" class="form-control @error('contact') is-invalid @enderror"
                            name="contact" value="{{ old('contact', $booking->contact) }}" required>
                        @error('contact')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <!-- Step 2: Address, Province, City, and Date Information -->
                <div class="step" id="step2">
                    <div class="form-group mb-3">
                        <label for="address">Address <span class="required">*</span></label>
                        <input id="address" type="text"
                            class="form-control @error('address') is-invalid @enderror"
                            name="address"value="{{ old('address', $booking->address) }}" required>
                        @error('address')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="row">
                            ...
                            <select name="province" class="form-select" id="province" required>
                                ...
                                @foreach (array_keys($cities) as $province)
                                    <option value="{{ $province }}"
                                        {{ old('province', $booking->province) == $province ? 'selected' : '' }}>
                                        {{ $province }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="city" class="form-select" id="city" required>
                                ...
                                @if (old('province', $booking->province))
                                    @foreach ($cities[old('province', $booking->province)] as $city)
                                        <option value="{{ $city['id'] }}"
                                            {{ old('city', $booking->city) == $city['id'] ? 'selected' : '' }}>
                                            {{ $city['name'] }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group mb-3">
                            <label for="site_visit_date" class="form-label">Date <span
                                    class="required">*</span></label>
                            <input id="site_visit_date" type="date"
                                class="form-control @error('site_visit_date') is-invalid @enderror"
                                name="site_visit_date"
                                value="{{ old('site_visit_date', $booking->site_visit_date) }}" required>
                        ...
                        <div class="button-container mb-3">
                            ...
                            <button type="submit" class="btn btn-info py-2 font-weight-bold">
                    ...
                    <div class="step" id="step3">
                        ...
                        <ul class="list-group mb-3">
                            <li class="list-group-item">Name: <span id="confirmName">{{ $booking->name }}</span></li>
                            <li class="list-group-item">Email: <span id="confirmEmail">{{ $booking->email }}</span>
                            </li>
                            <li class="list-group-item">Contact: <span
                                    id="confirmContact">{{ $booking->contact }}</span></li>
                            <li class="list-group-item">Address: <span
                                    id="confirmAddress">{{ $booking->address }}</span></li>
                            <li class="list-group-item">City: <span id="confirmCity">{{ $booking->city }}</span></li>
                            <li class="list-group-item">Province: <span
                                    id="confirmProvince">{{ $booking->province }}</span></li>
                            <li class="list-group-item">Date: <span
                                    id="confirmDate">{{ $booking->site_visit_date }}</span></li>
                        </ul>
                        <div class="button-container mb-3">
                            <button type="button" class="btn btn-transparent py-2 font-weight-bold"
                                id="prevStepBtn2">
                                <i class="fas fa-arrow-left"></i> Previous
                            </button>
                            <button type="submit" class="btn btn-info py-2 font-weight-bold">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function showStep(stepIndex) {
                steps.forEach((step, index) => {
                    step.style.display = index === stepIndex ? 'block' :
                        'none'; // Only show the current step
                });
            }


            // Update confirmation fields with current input values
            function updateConfirmation() {
                ...
                document.getElementById('confirmAddress').textContent = document.getElementById('address').value ||
                    "{{ $booking->address }}";
                document.getElementById('confirmCity').textContent = document.getElementById('city')
                    .selectedOptions[0].textContent || "{{ $booking->city }}";
                document.getElementById('confirmProvince').textContent = document.getElementById('province')
                    .selectedOptions[0].textContent || "{{ $booking->province }}";
                document.getElementById('confirmDate').textContent = document.getElementById('site_visit_date')
                    .value || "{{ $booking->site_visit_date }}";
            }
            // Navigation buttons
